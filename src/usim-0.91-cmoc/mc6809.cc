...
mc6809::mc6809() : mode(immediate), u(0), s(0), x(0), y(0), dp(0), acc(), a(acc.byte.a), b(acc.byte.b), d(acc.d), cc(), irq_pending(false)
{
	acc.d = 0;
	cc.all = 0;
	...
	write(0xFE00, 0x3b);  // RTI at $FE00 -> IRQ routine
	write(0xFE01, 0x12);  // NOP (reserved for jump instruction)
	write(0xFE02, 0x12);  // NOP (reserved for jump instruction)
	write_word(0xFFF8, 0xFE00);  // set $FE00 as IRQ routine
}
...
void mc6809::reset(void)
{
	...
	cc.bit.i = 0;		/* IRQ enabled */
	cc.bit.f = 0;		/* FIRQ enabled */
	s = 0xFE00;		/* FExx reserved for pseudo-ROM (e.g., IRQ routine) */
}
...
{
	/* If an IRQ is due to happen and is not masked, trigger it. */
	if (irq_pending && cc.bit.i == 0)
	{
		cc.bit.e = 1;  /* entire environment saved on the stack */
		help_psh(0xFF, s, u);  /* push all registers (including CC with E=1) */
		pc = read_word(0xFFF8);  /* jump to IRQ routine specified at FFF8:9 */
		irq_pending = false;
	}
	...
	if ((post & 0x80) == 0x00) {
		...
		switch (post & 0x1f) {
			...
			case 0x0b: case 0x1b:
				...
				break;
			case 0x0c: case 0x1c:
				// Old buggy line: addr = pc + extend8(fetch());
				// Fix by sarrazip 2009-07-11:
				addr = extend8(fetch());
				addr += pc;  // add the address that follows the whole instruction
				break;
			case 0x0d: case 0x1d:
				// Old buggy line: addr = pc + fetch_word();
				// Fix by sarrazip 2009-07-11:
				addr = fetch_word();  // this advanced PC by 2
				addr += pc;  // add the address that follows the whole instruction
				break;
			case 0x1f:
				addr = fetch_word();
