...
{
        // Source: http://techheap.packetizer.com/processors/6809/6809Instructions.html
	do_br((cc.bit.n & cc.bit.v) | (!cc.bit.n & !cc.bit.v));
}

void mc6809::lbge(void)
{
	do_lbr((cc.bit.n & cc.bit.v) | (!cc.bit.n & !cc.bit.v));
}

void mc6809::bgt(void)
{
        // Source: http://techheap.packetizer.com/processors/6809/6809Instructions.html
        do_br((cc.bit.n & cc.bit.v & !cc.bit.z) | (!cc.bit.n & !cc.bit.v & !cc.bit.z));
}

void mc6809::lbgt(void)
{
        do_lbr((cc.bit.n & cc.bit.v & !cc.bit.z) | (!cc.bit.n & !cc.bit.v & !cc.bit.z));
}
...
{
	// Source: http://techheap.packetizer.com/processors/6809/6809Instructions.html
	do_br(cc.bit.z | (cc.bit.n & !cc.bit.v) | ((!cc.bit.n) & cc.bit.v));
}

void mc6809::lble(void)
{
	do_lbr(cc.bit.z | (cc.bit.n & !cc.bit.v) | ((!cc.bit.n) & cc.bit.v));
}
...
void mc6809::help_sbc(Byte& x)
{
	...
	cc.bit.n = btst(x, 7);
	cc.bit.z = !x;
	#else  // new code taken from sdlmess0136/src/emu/cpu/m6809

	Word t = m;
	Word r = Word(x) - t - cc.bit.c;
	cc.bit.n = ((r & 0x80) != 0);
	cc.bit.z = ((r & 0xff) == 0);
	cc.bit.v = (((x ^ t ^ r ^ (r>>1)) & 0x80) != 0);
	cc.bit.c = ((r & 0x100) != 0);
	x = r;
